	...
	while (tx_limit < NO_TX) {
		...
		for (int i=0; i<6;i++) {
			if ((status = _xbegin()) == _XBEGIN_STARTED) {
				...
				//after 5 trials, acquire the lock
				if (i==5) {
					do {
						while(glock);
					}
					while (!__sync_bool_compare_and_swap(&glock, 0, 1));
//		pthread_mutex_lock(&mutex);
//					while(__sync_lock_test_and_set(&glock, 1));
						for (int i= 0; i < tx_len; i+=2) {
							if (array[loc[i]] > 50) {
								array[loc[i+1]] += 50;
								array[loc[i]] -= 50;
...
//								undo_log[undo_i].val.val = (void *) (array[loc[i]]);
//								undo_i++;
//								undo_log[undo_i].addr = (void **) &(array[loc[i + 1]]);
//								undo_log[undo_i].val.val = (void *) (array[loc[i + 1]]);
//								undo_i++;
//								array[loc[i + 1]] += 50;
//								array[loc[i]] -= 50;
//								p_wf.add(&array[loc[i]]);
//								p_wf.add(&array[loc[i + 1]]);
//							} else {
//								rf.add(&array[loc[i]]);
//							}
//
...
//										  undo_i--;
//										  *(undo_log[undo_i].addr) = undo_log[undo_i].val.val;
//										  //*(undo_log[undo_i-2]) = undo_log[undo_i-1];
//										  //undo_i -= 2;
//									  }
//	//					        	  if (45678 == (long)arg)printf("abort5\n");//TODO undo locations
//									  retry = true;
//									  for (int i = 0; i < 64*backoff; i++)
//											nop();
//									  goto retry_lbl;
//								  }
...
//	printf("Last complete = %d\n", last_complete.val);
	printf("all HTM = %d, global locking = %d\n", htm_c, glc);
}
